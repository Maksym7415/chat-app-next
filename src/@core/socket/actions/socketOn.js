...
export const socketOnTypingStateId = (chat) => {
	...
	const currentUserTyping = (user, conversationId) => {
		const fConversationType = (chatId, userTyping, isTyping = false) => {
			const conversation = conversationTypeState[chatId];
			const data = {
				...conversation,
				[userTyping.userId]: { ...userTyping, isTyping },
			};
			store.dispatch(
...
export const socketOnUserIdNewChat = (userId, router) =>
	socket.on(`userIdNewChat${userId}`, async (message, conversationId) => {
...
export const socketOnDeleteConversation = ({ params, router }) => {
	socket.on("deleteChat", ({ ids }) => {
		...
		store.dispatch(
			allActionsStore.removeConversationsIdAction({
				ids,
			}),
		);

		store.dispatch(
			allActionsStore.setConversationListAction(copyConversationsList),
		);
	});
};
...
export const socketOnClearConversation = () => {
	socket.on("clearChat", ({ ids }) => {
		...
		store.dispatch(
			allActionsStore.clearMessagesDataInConversationsIdAction({
				ids,
